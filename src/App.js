...
import AdminUserManager from './components/AdminUserManager'; // ← AGGIUNTO
import TxtFilesManager from './TxtFilesManager';
import AdminSidebarLayout from './components/AdminSidebarLayout';
import usersData from './components/AdminUserManager'; // Assicurati che users sia esportato o importa da dove serve
import negoziData from './data/negozi.json'; // <--- aggiungi questa import
...
const [availableStores, setAvailableStores] = useState(negoziData); // <--- usa negoziData come default
...
useEffect(() => {
  ...
  try {
   ...
   // Rimuovi duplicati basandoti sull'ID univoco (il filtro è già fatto dal backend)
...
console.log('📊 Dopo rimozione duplicati:', uniqueInvoices.length);
  ...
  const confirmDelivery = async (invoiceId, deliveryDate, noteErrori = '', customEmail = null) => {
    try {
      ...
      setSuccess(noteErrori ? '⚠️ Consegna confermata con note di errore! 📄 File TXT generato automaticamente.' : '✅ Consegna confermata con successo! 📄 File TXT generato automaticamente.');
...
>
  ...
  <div className="space-y-3 sm:space-y-0 sm:flex sm:items-end sm:space-x-4">
                            ...
                            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                              <button
                                onClick={() => {
                                  ...
                                  if (!dateInput.value) {
                                    setError('⚠️ Inserisci una data di consegna');
                                    setTimeout(() => setError(''), 3000);
                                    return;
                                  }
                                  // Usa email dal negozio selezionato
                                  const storeEmail = getStoreEmail(selectedStoreForConfirmation);
                                  confirmDelivery(invoice.id, dateInput.value, '', storeEmail);
                                }}
                                disabled={isLoading}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  setErrorModalInvoice({ ...invoice, deliveryDate: dateInput.value, confermato_da: getStoreEmail(selectedStoreForConfirmation) });
                                  setErrorNotes('');
                                }}
                                disabled={isLoading}
        ...
        // LAYOUT NORMALE PER OPERATOR (invariato)
        <>
        ...
        <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
          <button
            onClick={() => {
              ...
              confirmDelivery(errorModalInvoice.id, errorModalInvoice.deliveryDate, errorNotes);
